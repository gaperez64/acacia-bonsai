...
namespace acacia {
  ...
  inline int run_native_gr1_arm (const arg_parse_result& args, bool unreal,
    ...
    auto reduced = tlsf_gr1_reduce_v1 (pipeline.get (), &reduction_options,
                                       &reduction.value, &reduction_error);
    ...
    struct ArtifactOwner {
      std::array<char*, 4>& bytes;
      ~ArtifactOwner () { for (char* p : bytes) std::free (p); }
    } artifacts{bytes};
    int solved_unreal = 0;
    ...
    std::unique_ptr<Aig, decltype (&aig_free)> strategy (
        solve_gr1_oxidd_ex_with_certificate_v2 (game, &solved_unreal,
                                                 &solve_options, &certificate), aig_free);
    ...
    const TlsfGr1CheckStatus status =
        tlsf_gr1_check (&input, &check_options, &checked.value);
