...
namespace replay {
  ...
  constexpr Column column_schema[] {
      ...
      {"losing_proofs"},
      {"strategy_generators"},
      {"reopened_sources", CounterReset::attempt_na, 9},
      {"reopen_enqueues", CounterReset::attempt_na, 10},
      {"subsumption_scans", CounterReset::attempt_na, 11},
      {"subsumption_nodes_checked", CounterReset::attempt_na, 12},
      {"subsumption_nodes_invalidated", CounterReset::attempt_na, 16},
      {"scan_tombstones", CounterReset::attempt_na, 13},
      {"scan_prefilter_rejects", CounterReset::attempt_na, 14},
      {"scan_exact_compares", CounterReset::attempt_na, 15},
      {"subsumption_queries", CounterReset::attempt_na, 17},
      {"subsumption_hits", CounterReset::attempt_na, 18},
      {"subsumption_prefilter_skips", CounterReset::attempt_na, 19},
      {"losing_insertions", CounterReset::attempt_na, 20},
      {"losing_removals", CounterReset::attempt_na, 21},
      {"losing_antichain_size", CounterReset::attempt_na, 22},
      {"losing_antichain_peak", CounterReset::attempt_na, 23},
      {"proofs_total", CounterReset::attempt_na, 24},
      {"proofs_in_initial_cone", CounterReset::attempt_na, 25},
      {"dependency_list_len_sum", CounterReset::attempt_na, 26},
      {"dependency_list_len_max", CounterReset::attempt_na, 27},
      {"parse_ms"},
      {"eager_ms"},
