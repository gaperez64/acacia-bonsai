...
class k_bounded_safety_aut_2step_nosplit_crit {
  ...
  public:
    ...
    bool solve () {
      ...
      while (input_letters != bddfalse) {
        ...
        omega_actions bwd_actions, fwd_actions;
        ...
        while (output_letters != bddfalse) {
          ...
          const auto& [bwd, fwd] = compute_omega_action (one_input_letter & one_output_letter);
    ...
    auto critical_input_output_bwd_actions (const SetOfStates& F) {
      ...
      using omega_actions_pair_ref = std::reference_wrapper<const omega_actions_map::value_type>;
      std::set<omega_actions_pair_ref, ref_ptr_cmp<omega_actions_pair_ref>> C, Cbar;
      ...
      for (const auto& f : F) {
        ...
        for (const auto& elt : C) {
          ...
          for (const auto& action : output_actions)
            if (F.contains (omega (f, action, direction::forward))) {
              is_witness = false;
              break;
            }
          if (is_witness) {
  ...
  private:
    ...
    using omega_actions_map = std::map<omega_actions, std::set<bdd_t>>; // Maps a set of actions to a set of inputs.

    using omega_actions_ref = std::reference_wrapper<const omega_actions>;
    using omega_actions_ref_set = std::set<omega_actions_ref,
                                           ref_ptr_cmp<omega_actions_ref>>;

    omega_actions_map input_output_bwd_actions, input_output_fwd_actions;

    // This computes F = CPre(F), in the following way:
    // UPre(F) = F \cap F2
    // F2 = \cap_{i \in I} F1i
    // F1i = \cup_{o \in O} PreHat (F, i, o)
    void cpre_inplace (SetOfStates& F, const omega_actions_ref_set& io_actions) {
      SetOfStates F2;
      ...
      for (const auto& actions : io_actions) {
        ...
        for (const auto& action_vec : actions.get ()) {
          ...
          F1i.union_with (F.apply ([this, &action_vec] (const auto& m) {
            auto&& ret = omega (m, action_vec, direction::backward);
            if (verbose > 2)
    ...
    auto compute_omega_action (bdd letter) {
      omega_action_vec ret_bwd (aut->num_states ()),
    ...
    State omega (const State& m, const omega_action_vec& action_vec, direction dir) {
      State f (aut->num_states ());
