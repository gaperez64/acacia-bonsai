...
namespace acacia::spot_fastpath {
  ...
  inline fast_path_result deterministic_forbidden_fast_path (
    ...
    auto* const record = capture_search ? spot_records::active : nullptr;
    ...
    if (record) record->phase ("action-construction");
    ...
    const auto search_started =
        record ? spot_records::now () : detail::clock::time_point {};
    ...
    if (record)
      spot_records::put ("search_ms", std::to_string (
          std::chrono::duration<double, std::milli> (spot_records::now () - search_started).count ()));
  ...
  inline fast_path_result try_spot_nba_fast_path (const spot::twa_graph_ptr& aut_forbid,
    ...
    auto* const record = spot_records::active;
    if (record) {
      record->segment ("frozen-graph", "spot-fast");
    ...
    if (not is_plain_existential_buchi (aut_forbid)) {
      ...
      if (record) spot_records::put ("verification_kind", "gfg-precondition");
      ...
      if (record)
        spot_records::put ("verification_ms", std::to_string (
            std::chrono::duration<double, std::milli> (spot_records::now () - verification_started).count ()));
    }
    ...
    if (c == nba_fast_class::deterministic_buchi and detail::has_mode (mode, SPOT_FAST_DET)) {
      if (record) spot_records::put ("backend", "spot-fast-det");
      ...
      if (record) record->end_attempt (res.current_output_player_wins ? "WIN" : "LOSE", "spot-game");
    ...
    if (c == nba_fast_class::gfg_buchi and
      ...
      if (record) spot_records::put ("backend", "spot-fast-gfg");
      const auto solve_started = detail::clock::now ();
    ...
    if (record) record->end_attempt ("UNKNOWN", "declined");
    return result;
  }
