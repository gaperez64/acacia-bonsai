...
namespace acacia::solver_detail::equivariant {
  ...
  result<SetOfStates> solve_orbit_sweep (const spot::twa_graph_ptr& aut, VECTOR_ELT_T kmax,
    ...
    if (record) {
      record->segment ("frozen-graph", "backward-equivariant-sweep");
      record->begin_attempt (kmin);
      record->phase ("action-construction");
    }
    ...
    while (true) {
      ...
      for (const auto& orbit : *orbits) {
        ...
        if (not f.contains (state (init))) {
          if (record) record->end_attempt ("LOSE_K", "fixedpoint-refutation");
          if (k >= kmax) {
      ...
      if (not changed) {
        ...
        if (record) record->end_attempt ("WIN_K", "fixedpoint");
        std::optional<std::pair<VECTOR_ELT_T, SetOfStates>> win;
  ...
  result<SetOfStates> try_solve (spot::twa_graph_ptr aut, VECTOR_ELT_T kmax, VECTOR_ELT_T kmin,
    ...
    auto decline = [&] (const char* reason) {
      ...
      acacia::diagnostics::set_equivariant_decline (reason);
      if (record && attempt_started) record->end_attempt ("UNKNOWN", "declined");
    ...
    if (record) {
      record->segment ("frozen-graph", "backward-equivariant");
      record->begin_attempt (kmin);
      record->phase ("action-construction");
    }
    if (record) attempt_started = true;
