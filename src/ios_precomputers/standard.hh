      ...
      private:
        ...
        class bdd_it {
          public:
            ...
            auto operator* () const { return current_letter; }

            bool operator== (const bdd_it& rhs) const {
              return letter_set == rhs.letter_set && support == rhs.support;
            }
            bool operator!= (const bdd_it& rhs) const { return !(operator== (rhs)); }
          ...
          protected:
            virtual void get_next_letter () {
              if (letter_set == bddfalse)
                ...
                current_letter = bdd_satoneset (letter_set, support, bddtrue);
        ...
        // class that stores vector<pair<p, q>> and also the IO, iterating over it iterates over
        // the (p, q) pairs
        struct transitions_io_pair {
          public:
            ...
            auto& operator[] (size_t i) { return transitions[i]; }
        };
        ...
        class ios_it : public bdd_it {
          public:
            ...
            auto operator* () const {
              // return current_io (vector<pair<p, q>>) AND the IO compatible with I that gave this
              // action
