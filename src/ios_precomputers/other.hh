...
for (auto crossing : crossings) {
  ...
  while (fvars != bddfalse) {
    ...
    if (input_and_not_crossing_inputs != bddfalse) {  // Split
      ...
      debug ("splitting " << bdd_to_formula (input) << " into "
                          << bdd_to_formula (input_and_crossing_inputs) << " and "
                          << bdd_to_formula (input_and_not_crossing_inputs));
      ;
      ...
      ASSERT (input_and_not_crossing_inputs ==
              bdd_existcomp (input_and_not_crossing_inputs, input_support));
      ASSERT (input_and_crossing_inputs ==
              bdd_existcomp (input_and_crossing_inputs, input_support));
    }
    else
