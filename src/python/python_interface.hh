 ...
 */
Game* create_twa (const std::string& formula, const std::vector<std::string>& input_aps,
                  const std::vector<std::string>& output_aps, bool unreal_x_formula = false);
...
class winreg_iterator {
  ...
  public:
    explicit winreg_iterator (const winreg_type& s) : cur {s.begin ()}, end {s.end ()} {}
...
class WinningRegion {
  ...
  public:
    WinningRegion (VECTOR_ELT_T k, winreg_type&& r) : k {k}, region {std::move (r)} {}
    ...
    [[nodiscard]]
    VECTOR_ELT_T get_k () const {
      return k;
    }

    [[nodiscard]]
    const winreg_type& get_region () const {
      return region;
    }
};
...
class GameResult {
  ...
  public:
    ...
    GameResult (VECTOR_ELT_T k, winreg_type&& r) : region {std::in_place, k, std::move (r)} {}
 ...
 */
vector_wrapper* successor (Game& game, const vector_wrapper& v,
