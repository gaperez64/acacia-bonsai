...
static void prep_unreal_formula (spot::formula& formula,
  ...
  auto rec = [&input_aps] (auto&& self, spot::formula m) -> spot::formula {
    if (m.is (spot::op::ap) and (std::ranges::find (input_aps, m.ap_name ()) != input_aps.end ()))
...
static spot::twa_graph_ptr build_twa (spot::formula& formula, spot::bdd_dict_ptr dict) {
...
Game* create_twa (const std::string& formula_str, const std::vector<std::string>& input_aps,
                  const std::vector<std::string>& output_aps, bool unreal_x_formula) {
...
void preprocess_aut_standard (Game& game, int k_max) {
  aut_preprocessors::standard::make (game.twa, game.inputs, game.outputs, k_max) ();
}

void preprocess_aut_surely_losing (Game& game, int k_max) {
  aut_preprocessors::surely_losing::make (game.twa, game.inputs, game.outputs, k_max) ();
}
...
static std::optional<std::pair<VECTOR_ELT_T, winreg_type>> solve_game_python (
  ...
  auto skn = k_bounded_safety_aut_detail<SpecializedDownset, IOsPrecomputationMaker, ActionerMaker,
                                         InputPickerMaker> (aut, k_min, k_max, k_inc, all_inputs,
                                                            all_outputs, IOsPrecomputationMaker (),
                                                            ActionerMaker (), InputPickerMaker ());
  return skn.solve ();
}

GameResult* solve_acacia_safety_game (Game& game, int k_max, int k_min, int k_inc) {
  auto res = solve_game_python (game.twa, (VECTOR_ELT_T) k_max, (VECTOR_ELT_T) k_min,
                                (VECTOR_ELT_T) k_inc, game.inputs, game.outputs);
  if (res.has_value ()) {
    auto& [k, region] = *res;
...
std::vector<std::string> get_input_aps (const Game& game) { return game.input_aps; }

std::vector<std::string> get_output_aps (const Game& game) { return game.output_aps; }

unsigned num_states (const Game& game) { return game.twa->num_states (); }

unsigned initial_state_number (const Game& game) { return game.twa->get_init_state_number (); }
...
vector_wrapper* make_vector (const Game& game, const std::vector<int>& entries) {
...
static bdd cube_from_assignment (const Game& game, const std::vector<std::string>& true_aps,
  ...
  for (const auto& ap : true_aps)
    encode (ap, true);
  for (const auto& ap : false_aps)
    encode (ap, false);
  return cube;
}

vector_wrapper* successor (Game& game, const vector_wrapper& vw,
                           const std::vector<std::string>& true_aps,
                           const std::vector<std::string>& false_aps, int k_cap) {
  auto aut = game.twa;
  ...
  for (unsigned src = 0; src < n; ++src) {
    ...
    for (auto& e : aut->out (src)) {
      if ((e.cond & cube) == bddfalse)
        continue;
      unsigned dst = e.dst;
      VECTOR_ELT_T candidate = std::min (
          K, (VECTOR_ELT_T) (v[src] + (VECTOR_ELT_T) (aut->state_is_accepting (dst) ? 1 : 0)));
