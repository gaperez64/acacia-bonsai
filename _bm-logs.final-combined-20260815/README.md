...
This directory is the authoritative final gate set for the native TLSF,
symmetry-certificate, and gate-repair branch.  The candidate and baseline use
...
- `g3/`: all landing reports pass.  Candidate results are 90/94 and 231.630
  ...
  `syntcomp25/panel`.
- `g5-enum*`: final-binary parity passes on all nine enum-bearing common TLSF
  inputs after the enum compatibility repair, with zero disagreements,
  errors, or resource limits.  This is the affected-input closure for the full
  1,579-instance G5 run in `_bm-logs.stepc-tlsf-parity-20260814/`.
