...
\While{true}{
  ...
  \lIf{no action is returned}{\Return \((\vinit \stackrel{\text{\smash{?}}}{\in} S)\)}
...
implementations:
\begin{itemize}
\item Standard vector and array types (\cinline{std::vector},
...
(\cinline{std::bitset}) to provide a type that combines both boolean and integer
...
% LocalWords:  iteratively Downset downset
