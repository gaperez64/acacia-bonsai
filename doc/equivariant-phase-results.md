...
## 2026-08-02 two-gate follow-up

The detector now retains the largest structurally verified symmetric client
component instead of abandoning recognition when a swap involving client 0
fails.  The minimum block payoff gate is also configurable and defaults to 4
instead of 8.  This admits the measured $S_{n-1}$ components in AMBA and
load-balancer automata as well as the four-block plain-arbiter layout.

The full 994-instance campaign was repeated under the same 17 s, one-job,
8 GiB/no-swap protocol in both configuration orders.
...
The counterbalanced equivariant win is 39.237 s (0.625%).  Both campaigns use
identical instance universes and have no failures or wrong verdicts.  The first
order has one favorable timeout-boundary difference on `detector_unreal16`;
the reversed order solves 823 instances in both modes and still favors the
equivariant build by 35.387 s, so the conclusion is not carried by that case.

The new partial-group market is real: `amba_decomposed_arbiter6` repeats at a
1.92 s mean versus 13.75 s classic-only, and `load_balancer7` at 11.05 s versus
13.63 s.  The existing `arbiter_on_inpchange4` control remains a 2.39 s versus
12.96 s win.  The limit is equally clear: AMBA sizes 7–8 and plain `arbiter6`
through `arbiter8` still time out in both modes.  The gate fixes improve the
corpus result without making the largest four-block cases tractable.
