...
Henceforth, we fix a universal safety automaton $\calS^K_\calN = (\calF,f_0,I
\times O, T,U)$.  Previously, we have given a definition for $\upre$ in the
context of any safety game. For the safety game played on $\calS^K_\calN$ and
for a set $F \subseteq \calF$, $\upre$
...
the sequel we will argue that if $F$ is upwards closed and $A = \lfloor F
\rfloor$ then $\upre(F)$ can be
replaced by the closure of $\underline{\upre} : \frakA \to \frakA$,
...
\[
  ...
  \},
\]
from~\eqref{eqn:A0io}, is closed suffices for the lemma to hold.
...
\begin{proof}
  ...
  \]
  Further, we know $m \preceq T(f,\langle i, o \rangle)$ from our initial
  assumption. Hence $f'$ is also in the set.\qed
\end{proof}
