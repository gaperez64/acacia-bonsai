...
\begin{proposition}[From~\cite{acacia0}]
  Let $m \in \calF$, $i \in I$, $o \in O$. Then, the set
  \(
  ...
  \[
    \closeup{
      \{ f \in \calF \mid f \preceq \pseudopre(m,i,o), m \preceq T(f, \langle
      i, o \rangle) \preceq T(\pseudopre(m,i,o),\langle i, o \rangle) \}
    }.
  \]
\end{proposition}
\begin{proof}[Sketch]
  By \Cref{lem:closed-pre}, we get that the first set is closed. Hence, it
  suffices to argue that for any $g$ in the first set there is some $f \in
  \calF$ such that the following hold.
  \begin{itemize}
    \item $f \preceq g$
    \item $f \preceq \pseudopre(m,i,o)$
    \item $m \preceq T(f,\langle i, o \rangle) \preceq T(\pseudopre(m,i,o), \langle i, o \rangle)$
  \end{itemize}
  It suffices to argue that the first antichain is a subset of the
  second antichain. Consider some $f \in \calF$ such that
  $m \preceq T(f,\langle i, o\rangle)$. From \Cref{lem:order-pseudopre}
  \Cref{itm:order-pre} it follows that
  \[
    \pseudopre(m,i,o) \preceq \pseudopre(T(f,\langle i, o \rangle), i, o).
  \]
  Furthermore, we know that
  \[
    asdf
  \]
  by \Cref{lem:order-pseudopre} \Cref{itm:order-postpre}.
\end{proof}
