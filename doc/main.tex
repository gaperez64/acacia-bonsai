...
as soon as a B\"uchi state is reached,\mcnote{When I suggested using ``Büchi''
  rather than ``accepting,'' I was thinking about using the former
  \emph{everywhere}.  Thoughts?} the word should be
...
losing states from $\calS^K_\calN$ are \emph{$\preceq$-upwards closed}\mcnote{I'm not
  sure about the use of the adverb \emph{upwards}, instead of the adv/adj
  \emph{upward}; can we use upward throughout?}. We further
...
Henceforth, we fix a universal\mcnote{Let's be consistent: Do we want to include
  the term ``universal'' for this automaton, or just keep ``deterministic,'' or
  even just say ``safety automaton?''  Currently, it fluctuates.} safety
...
the next lemma, we will argue that if $F$ is upwards closed and $A = \lfloor F
...
\end{align}
Both $\underline{\upre}^n(A)$, for $n \in \mathbb{N}$, and
$\underline{\upre}^*(A)$
are defined as for $\upre$.\mcnote{Do we need to argue that the star version
  exists?}
...
\begin{lemma}\label{lem:uupre}
  ...
  \)
\end{lemma}
From \Cref{pro:antichains} and the definitions of $\upre$ and $\underline{\upre}$ it
follows that proving 
\[
  \{f \in \calF \mid T(f,\langle i, o \rangle) \in F\}
  = \bigcup_{m \in A} \{ f \in \calF \mid m \preceq T(f, \langle i,o \rangle)
  \},
\]
from~\eqref{eqn:A0io}, is closed suffices for the lemma to hold. \mcnote[inline]{It seems
  that two statements are crammed in the previous sentence: the equality of these two
  sets, which is arguably quite immediate, and the claim that it suffices that
  it be closed for the lemma to hold.  I'd rather split these two statements.}

\begin{lemma}
  Let $m \in \calF$ and $(i,o) \in I \times O$. Then $\{ f \in \calF \mid m
  \preceq T(f, \langle i,o \rangle) \}$ is closed.\mcnote{This lemma could be incorporated as a
    fact in the proof of \Cref{lem:uupre}, no?}
\end{lemma}
\begin{proof}
...
\gpnote{Can we just compute the predecessors of the minima and take the
minima of that of that?}
