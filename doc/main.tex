...
$\pseudopre(m,i,o)$ where $m \in A, i \in I, o \in O$. Importantly, we need to
make sure we ignore $f = \pseudopre(f,i,o)$ if $f(q) = \bot$ for all $q \in
Q$.
%
\begin{proposition}[From~\cite{acacia0}]
  Let $F \subseteq \calF$ be a closed set represented by the antichain $A = \lfloor
  F \rfloor$, $i \in I$, and $o \in O$. The following holds:
  \[
    \left\lfloor \bigcup_{m \in A} \{ f \in \calF \mid m \preceq T(f, \langle i,
    o \rangle \} \right\rfloor
    =
    \left\lfloor \bigcup_{m \in A} \{ f \in \calF \mid f = \pseudopre(m,i,o) \} \right\rfloor.
  \]
\end{proposition}
\begin{proof}[Sketch]
  Using \Cref{lem:closed-pre} and the definition of $\pseudopre$ it is easy to
  argue that 
  \[
    \pseudopre(m,i,o) \in \{ f \in \calF \mid m \preceq T(f, \langle i, o
    \rangle \}
  \]
  for all $m \in A$, all $i \in I$, and all $o \in O$. Hence, it suffices to
  prove that for all $f \in \calF$ such that $m \preceq T(f, \langle i, o
  \rangle)$ we have $\pseudopre(m,i,o) \preceq f$. However, the latter is also
  easy to prove using the definition of $\pseudopre$.\qed
\end{proof}
