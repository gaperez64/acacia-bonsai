...
\begin{align*}
  ...
  %
  A^{(1)}_{i} := {} &
  \left\lceil \bigsqcup_{o \in O} A^{(0)}_{i,o} \right\rceil\\
  %
  A^{(2)} := {} &
  \left\lceil \bigcap_{i \in I} A^{(1)}_i \right\rceil\\
  %
  \underline{\cpre}(A) := {} &
...
\begin{theorem}
  A state $f \subseteq \calF$ is winning for \eve if and only if $f \in
