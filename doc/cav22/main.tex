...
\author{Micha\"el Cadilhac\inst{1}%\orcidID{1234-5678-9012}
...
two terms is slightly more complex: An \emph{input} (\resp \emph{output}) is a
boolean combination of symbols of \(I\) (\resp $O$) and it is \emph{pure} if it is
a \emph{conjunction} in which \emph{all} the symbols in \(I\) (\resp $O$) appear;
e.g., with \(I = \{i_1, i_2\}\), the expressions \(\top\) (true), \(\bot\) (false), and
...
We use \(i, j\) to denote inputs and \(x, y\) for IOs.  Two IOs \(x\) and \(y\) are
\emph{compatible} if \(x \land y \neq \bot\).
...
\(I \cup O\).  We assume, throughout this paper, the existence of some automaton
\(\cA\).
...
\section{Checking nonrealizability}\label{sec:unreal}
...
thousand LTL formulas, and candidate tools are ran with a time limit of one hour
...
ltlsynt~\cite{duret.16.atva2}.  The benchmarks were completed on a headless
...
\item Preprocessing of the automaton (\Cref{sec:preproc}). This seems to have
  ...
  the performances were \emph{worse} with automaton preprocessing, owing to the
...
\item Boolean states (\Cref{sec:implem-bool}). Using boolean states boosted
  performances by a marginal amount when SIMD was activated, but had a more
...
\item Nonrealizability (\Cref{sec:unreal}). Computation of the nonrealizability
  ...
  elected to create two processes for nonrealizability, one for each option,
  allowing for parallel computation on the two resulting automata.
\end{compactitem}

\paragraph{Acacia-Bonsai and foes.}  The following graph shows the performance
of the tools.  Since these tools tend to solve a lot of instances under one
second, we elected to present this graphic with a logarithmic y-axis.  Thanks to
this, the cost of entry of SIMD instructions is also emphasized.
...
%; note that all compilation optimization available were turned on
%for all tools, and that execution conditions were the same for all.
...
this careful reimplementation surpasses the performances of the original
