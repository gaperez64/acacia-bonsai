...
\institute{\anon{DePaul University, USA}\\
 \anon{\email{michael@cadilhac.name}} \and
 \anon{University of Antwerp -- Flanders Make, Belgium}\\
...
\While{\(V\) is nonempty}{
  ...
  \For{every \((p,x,q) \in \delta\)}{
    $y \leftarrow x$\;
    \While(\tcp*[h]{\rm Iterating through $x$'s minterms focusing on inputs}){$y \neq \bot$}{
      Let \(i\) be a pure input compatible with \(y\)\;
      \(o' \leftarrow \exists I.\.x \land i\) \tcp*[h]{\rm Extract nonpure output compatible with \(i\)}\;
      Add $(\langle p, o\rangle, o \land i, \langle q, o'\rangle)$ to $\Delta$\;
...
\item Unrealizability (\Cref{sec:unreal}). The following figure shows how the
  ...
  \begin{figure}[h!]
    \centering
    \includegraphics[width=0.7\textwidth]{unreal}
...
\anon{\url{https://github.com/gaperez64/acacia-bonsai/}}.
