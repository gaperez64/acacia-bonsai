...
## Selection: N-a and N-b, both caps
...
under `benchmarking/witness-lifting-20260918/opening/{120s,17s}/`, uses
...
combine the best rows of N-a and N-b into a synthetic series.
...
TACAS23 at 120 s uses the existing `run-subset.py` `acacia1x` adapter and
...
V1_OUT="$CAMPAIGN/full/120s/TACAS23"
mkdir -p "$V1_OUT"
...
python3 benchmarking/run-subset.py --tool acacia1x --bin "$V1" \
  --list "$LIST" --instances-dir "$PAIRS" --timeout 120 \
  --systemd-scope --memory-max 8G --memory-swap-max 0 \
  --csv "$V1_OUT/v1-convertible.csv"
python3 - "$LIST" "$V1_OUT/v1-convertible.csv" "$V1_OUT/v1-cap120.csv" <<'PY'
...
The two ltlsynt legs are
`benchmarking/witness-lifting-20260918/opening/{120s,17s}/ltlsynt-cap*.csv`;
...
The full N loop exports its own CSV and `.raw.tsv` sidecar. B's epoch-1
exports already exist at
`benchmarking/witness-lifting-20260918/opening/{120s,17s}/epoch-1/B-cap*-epoch1.csv`.
To regenerate any B export from its raw row, use:

```bash
for cap in 120 17; do
  dir="$ROOT/benchmarking/witness-lifting-20260918/opening/${cap}s/epoch-1"
  python3 benchmarking/run-syntcomp26-coverage.py export-cactus \
    --summary "$dir/B-cap$cap-epoch1-summary.tsv" --list "$LIST" --cap "$cap" \
    --output "$dir/B-cap$cap-epoch1.csv"
done
```

The external `run-subset.py` legs are already four-column cactus CSVs: the
new `v1-cap120.csv` above, the archived 17 s v1 CSV, and both pinned ltlsynt
CSVs. No `export-cactus` transformation applies to those legs. Run this
...
for CAP in 120 17; do
  ...
  mkdir -p "$REPORT"
  LTLSYNT="$ROOT/benchmarking/witness-lifting-20260918/opening/${CAP}s/ltlsynt-cap$CAP.csv"
  NEW="$CAMPAIGN/full/${CAP}s/$CHOSEN/N-cap$CAP.csv"
  if [ "$CAP" = 120 ]; then
    TACAS23="$CAMPAIGN/full/120s/TACAS23/v1-cap120.csv"
  else
    TACAS23="$ROOT/benchmarking/plots/three-way-full-20260905/syntcomp26-full-v1.csv"
  fi
  python3 benchmarking/cactus-report.py \
    --csv "ltlsynt=$LTLSYNT" --csv "Acacia TACAS23=$TACAS23" \
    --csv "New Acacia=$NEW" \
...
| Leg | 120 s cap | 17 s cap |
|---|---:|---:|
| B full, epoch 1 / epoch 2 | 38,263 / 38,272 s | 6,691 / 6,687 s |
| B eligible subset, epoch 1 / epoch 2 | 6,673 / 6,671 s | 1,004 / 1,003 s |
| N-a selection proxy (91 IDs) | 8,171 s (2.27 h) | 1,197 s (20.0 min) |
| N-b selection proxy (91 IDs) | 9,580 s (2.66 h) | 1,382 s (23.0 min) |
| N-a full proxy (1,524 IDs) | 39,815 s (11.06 h) | 6,936 s (1.93 h) |
| N-b full proxy (1,524 IDs) | 41,223 s (11.45 h) | 7,121 s (1.98 h) |
| ltlsynt retained leg | 28,197 s (7.83 h; reuse) | 4,812 s (1.34 h; reuse) |
| TACAS23 retained 17 s rows, `Σ min(seconds, cap)` | 11,281 s lower bound | 11,229 s (3.12 h; reuse) |
| TACAS23 120 s planning proxy: charge 625 old TIMEOUTs at 120 s | 75,604 s (21.0 h) | — |

The TACAS23 120 s lower bound uses censored 17 s TIMEOUT observations and
cannot predict how many solve under 120 s. Its 21-hour proxy charges those
625 rows at the new cap and carries over the other observed times. All four N
selection legs, both N full legs, and TACAS23 120 s require actual measurements.
