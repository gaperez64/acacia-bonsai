...
1. **Strip.** The production path (scripts/acacia-lift-portfolio.py → python -m acacia_lift.runner,
   ...
   per-family stable_from/arity/role tables. Remove them from scripts/acacia_lift/ (delete the
   ...
   evidence) may keep working only if it lives entirely outside scripts/acacia_lift/ and is
...
3. **Anti-hardcoding guard.** A pytest (tests/pytest/) that fails if any module under
   scripts/acacia_lift/ or the wrapper contains any of the 29 family labels found in the old
